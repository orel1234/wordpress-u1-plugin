...
async function validateMapping(type, primary, fieldValues, rootValues) {
  ...
  // A name U1 itself created cannot appear in a mapping — it is circular.
  //
  // `#u1-anchor-f9u36-1` is generated by u1.fix.* while decorating the trigger;
  // `u1st-tabbable-element` is added at the same moment. Neither is in the
  // site's own HTML. A mapping written against one resolves perfectly in a
  // panel looking at an already-decorated page and matches NOTHING on the
  // client's site at load, where the fix has not run yet. Generation refuses
  // them now, but a pasted selector, an edit, or a model's answer can still
  // carry one — and it looks like the best answer available.
  for (const [k, v] of Object.entries(map)) {
    const parts = String(v).match(/[#.][\w-]+/g) || [];
    const bad = parts.filter((t) => /^[#.]u1(st)?-/i.test(t));
    if (bad.length) {
      notes.push({ level: 'err', msg:
        `“${k}” uses ${bad.join(', ')}, which U1 itself adds to the page while ` +
        `it runs. It is not in the site's HTML, so this matches on your screen ` +
        `now and matches nothing on the live site at load. Use the element's own ` +
        `class or id instead.` });
    }
  }
