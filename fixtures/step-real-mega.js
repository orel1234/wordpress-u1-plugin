...
Mega.widgets = {
  ...
  auditCases() {
    ...
    if (acInput && acList) {
      ...
      const paint = () => {
        ...
        acList.innerHTML = hits.map(m =>
          `<li style="padding:6px 10px;cursor:pointer">${m}</li>`).join('');
        acInput.setAttribute('data-expanded', String(hits.length > 0));
      };
      ...
      acList.addEventListener('click', e => {
        ...
        acList.innerHTML = '';
        acInput.setAttribute('data-expanded', 'false');
      });
      acInput.addEventListener('blur', () => setTimeout(() => {
        acList.innerHTML = ''; acInput.setAttribute('data-expanded', 'false');
      }, 150));
    }
    ...
    if (rtBtn && rt) {
      const showRt = () => { rt.classList.toggle('is-hidden', !!(false)); };
      const hideRt = () => { rt.classList.toggle('is-hidden', !!(true)); };
    ...
    if (checkBtn && spin) {
      checkBtn.addEventListener('click', () => {
        spin.classList.toggle('is-hidden', !!(false));
        if (checkOut) checkOut.textContent = '';
        setTimeout(() => {
          spin.classList.toggle('is-hidden', !!(true));
    ...
    if (pager) pager.addEventListener('click', (e) => {
      ...
      Store.utils.qsa('button', pager).forEach((x) => x.removeAttribute('data-current'));
      b.setAttribute('data-current', 'page');
    });
    ...
    if (dateBtn && datePanel) {
      ...
      dateBtn.addEventListener('click', () => {
        datePanel.classList.toggle('is-hidden');
        if (!datePanel.classList.contains('is-hidden') && !datePanel.children.length) paint();
      });
      datePanel.addEventListener('click', (e) => {
        ...
        datePanel.classList.toggle('is-hidden', !!(true));
      });
    }
    ...
    if (vt) vt.addEventListener('click', (e) => {
      ...
      Store.utils.qsa('[data-vpanel]', vt).forEach((p) => {
        p.classList.toggle('is-hidden', !!(p.getAttribute('data-vpanel'))) !== tab.getAttribute('data-vtab');
      });
    });
    ...
    if (ht) ht.addEventListener('click', (e) => {
      ...
      ['hashPanelSpecs', 'hashPanelShipping', 'hashPanelWarranty'].forEach((p) => {
        const elp = el(p); if (elp) elp.classList.toggle('is-hidden', !!(p !== id));
      });
    });
