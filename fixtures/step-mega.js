...
Mega.render = {
    ...
    host.innerHTML = Mega.MENU.map((item, i) => `
      <li class="mega-nav__item">
        <button class="mega-nav__trigger ${item.hot ? 'mega-nav__trigger--hot' : ''}"
                type="button" aria-expanded="false" aria-controls="megaPanel${i}" data-mega-trigger="${i}">
          ${esc(item.label)} <span class="mega-nav__caret" aria-hidden="true">▾</span>
        </button>
        <div class="mega-panel" id="megaPanel${i}" data-mega-panel="${i}" hidden>
          ${item.cols.map(col => `
            <div>
  ...
  hero() {
    ...
    track.innerHTML = Mega.SLIDES.map((s, i) => `
      <div class="hero-slide ${i === 0 ? 'hero-slide--active' : ''}" data-slide="${i}"
           role="group" aria-roledescription="slide" aria-label="Slide ${i + 1} of ${Mega.SLIDES.length}">
  ...
  dealTabs() {
    ...
    host.innerHTML = Mega.DEAL_TABS.map((t, i) => `
      <button class="tab-bar__btn" type="button" role="tab" id="dealTab-${t.id}"
              aria-controls="dealPanel" aria-selected="${i === 0}" data-deal-tab="${t.id}">${esc(t.label)}</button>`).join('');
  },
  ...
  stores() {
    ...
    host.innerHTML = Mega.STORES.map(([name, addr, hours, tags, open], i) => `
      <button class="locator__item" type="button" data-store="${i}" aria-current="${i === 0}">
  ...
  faqTabs() {
    ...
    host.innerHTML = Mega.FAQ_TABS.map((t, i) => `
      <button class="tab-bar__btn" type="button" role="tab" id="faqTab-${t.id}" aria-selected="${i === 0}"
              aria-controls="faqPanel" data-faq-tab="${t.id}">${esc(t.label)}</button>`).join('');
  },
      ...
      <div class="accordion__item">
        <h3>
          <button class="accordion__trigger" type="button" aria-expanded="false"
                  aria-controls="faqBody-${tabId}-${i}" data-accordion>
            <span>${esc(q)}</span><span class="accordion__icon" aria-hidden="true">+</span>
          </button>
        </h3>
        <div class="accordion__panel" id="faqBody-${tabId}-${i}" hidden>${esc(a)}</div>
      </div>`).join('');
  },
...
Mega.widgets = {
  ...
  megaMenu() {
    ...
    const closeAll = except => {
      Store.utils.qsa('[data-mega-panel]', nav).forEach(panel => {
        ...
        panel.hidden = true;
        nav.querySelector(`[data-mega-trigger="${panel.dataset.megaPanel}"]`).setAttribute('aria-expanded', 'false');
      });
    };
  ...
  finder() {
    ...
    tabs.forEach(tab => tab.addEventListener('click', () => {
      tabs.forEach(t => {
        ...
        el(t.dataset.finderTab).hidden = !selected;
      });
    }));
  ...
  dealTabs() {
    ...
    host.addEventListener('click', e => {
      ...
      if (!btn) return;
      Store.utils.qsa('[data-deal-tab]', host).forEach(b => b.setAttribute('aria-selected', String(b === btn)));
      el('dealGrid').setAttribute('aria-labelledby', btn.id);
  ...
  locator() {
    ...
    list.addEventListener('click', e => {
      ...
      if (!btn) return;
      Store.utils.qsa('[data-store]', list).forEach(b => b.setAttribute('aria-current', String(b === btn)));
  ...
  faq() {
    ...
    tabs.addEventListener('click', e => {
      ...
      if (!btn) return;
      Store.utils.qsa('[data-faq-tab]', tabs).forEach(b => b.setAttribute('aria-selected', String(b === btn)));
  ...
  chat() {
    ...
    let i = 0;
    el('chatToggle').addEventListener('click', () => { win.hidden = !win.hidden; if (!win.hidden) el('chatInput').focus(); });
    el('chatClose').addEventListener('click', () => { win.hidden = true; });
  ...
  modals() {
    Store.utils.qsa('[data-modal-open]').forEach(btn => btn.addEventListener('click', () => {
      el(btn.dataset.modalOpen).hidden = false;
    }));
    ...
    document.addEventListener('keydown', e => {
      ...
      Store.utils.qsa('.modal').forEach(m => { m.hidden = true; });
    });
  },
  ...
  auditCases() {
    ...
    if (rmBtn && rmPanel) {
      rmBtn.addEventListener('click', () => {
        ...
        rmBtn.setAttribute('aria-expanded', String(!rmPanel.hidden));
      });
    }
    ...
    if (miniStrip) {
      miniStrip.addEventListener('click', e => {
        ...
        Store.utils.qsa('[data-mini-panel]', miniStrip).forEach(p => {
          p.hidden = p.getAttribute('data-mini-panel') !== tab.getAttribute('data-mini-tab');
        });
      });
    }
    ...
    if (cityIn && cityList) {
      ...
      cityIn.addEventListener('input', () => {
        ...
        cityList.innerHTML = hits.map((c) =>
          `<li role="option" style="padding:6px 10px;cursor:pointer">${c}</li>`).join('');
        ...
        cityIn.setAttribute('aria-expanded', String(hits.length > 0));
      });
      cityIn.addEventListener('blur', () => setTimeout(() => {
        cityList.style.display = 'none'; cityIn.setAttribute('aria-expanded', 'false');
      }, 150));
