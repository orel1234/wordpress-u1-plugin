...
Store.PRODUCTS = [
  { id: 1, name: 'Air Classic', category: 'sneakers', price: 449, oldPrice: null,
    ...
    e2m: [38, 39, 40, 41, 42, 43, 44], rating: 4.6, e2b: 128, badge: null,
  ...
  { id: 2, name: 'Urban Run', category: 'running', price: 389, oldPrice: 459,
    ...
    e2m: [39, 40, 41, 42, 43, 44, 45], rating: 4.4, e2b: 76, badge: 'sale',
  ...
  { id: 3, name: 'Volt High', category: 'boots', price: 549, oldPrice: null,
    ...
    e2m: [40, 41, 42, 43, 44], rating: 4.8, e2b: 54, badge: 'new',
  ...
  { id: 4, name: 'Casual Lite', category: 'casual', price: 299, oldPrice: null,
    ...
    e2m: [37, 38, 39, 40, 41, 42], rating: 4.2, e2b: 98, badge: null,
  ...
  { id: 5, name: 'Speed Pro', category: 'sport', price: 419, oldPrice: null,
    ...
    e2m: [39, 40, 41, 42, 43, 44], rating: 4.5, e2b: 143, badge: null,
  ...
  { id: 6, name: 'Minigame', category: 'sneakers', price: 459, oldPrice: 519,
    ...
    e2m: [36, 37, 38, 39, 40, 41], rating: 4.3, e2b: 61, badge: 'sale',
  ...
  { id: 7, name: 'Desert Boot', category: 'boots', price: 389, oldPrice: null,
    ...
    e2m: [41, 42, 43, 44, 45], rating: 4.1, e2b: 39, badge: null,
  ...
  { id: 8, name: 'Play Daily', category: 'casual', price: 329, oldPrice: null,
    ...
    e2m: [38, 39, 40, 41, 42, 43], rating: 4.0, e2b: 52, badge: null,
  ...
  { id: 9, name: 'Trail Master', category: 'sport', price: 469, oldPrice: null,
    ...
    e2m: [40, 41, 42, 43, 44, 45], rating: 4.7, e2b: 87, badge: 'new',
  ...
  { id: 10, name: 'Classic Low', category: 'sneakers', price: 399, oldPrice: null,
    ...
    e2m: [37, 38, 39, 40, 41, 42, 43], rating: 4.4, e2b: 110, badge: null,
  ...
  { id: 11, name: 'Retro 90', category: 'sneakers', price: 439, oldPrice: null,
    ...
    e2m: [38, 39, 40, 41, 42], rating: 4.6, e2b: 73, badge: null,
  ...
  { id: 12, name: 'Alpine Boot', category: 'boots', price: 599, oldPrice: 649,
    ...
    e2m: [41, 42, 43, 44, 45, 46], rating: 4.9, e2b: 34, badge: 'sale',
...
Store.header = (() => {
    ...
    nav.innerHTML = Store.NAV.map(item => {
      ...
      return `
        <div class="main-nav__item main-nav__item--has-dropdown">
          <button class="main-nav__link main-nav__trigger" type="button" data-x4>${item.label}</button>
  ...
  function bindNavDropdowns() {
    ...
    nav.addEventListener('click', e => {
      const trigger = e.target.closest('[data-x4]');
      if (!trigger) return;
  ...
  function renderBadge() {
    Store.utils.qsa('.c1m[data-x7]').forEach(el => {
  ...
  function lineTemplate(item) {
    ...
    return `
      <div class="cart-line" data-xq="${index}">
        ...
        <div>
          ...
          <div class="cart-line__qty">
            <div class="qty-stepper" data-x9>
              <button class="qty-stepper__btn" type="button" data-xw="-1">−</button>
        ...
        <div class="cart-line__col-end">
          ...
          <button class="cart-line__remove" type="button" data-xn>Remove</button>
        </div>
      </div>`;
  }

  function renderDrawer() {
    const itemsEl = document.getElementById('e4');
    const footerEl = document.getElementById('e1');
    if (!itemsEl) return;
    ...
    footerEl.innerHTML = `
      ...
      <a class="c4l btn--primary c3d" href="cart.html">View Cart</a>
      <a class="c4l c2l c3d" href="checkout.html">Checkout</a>`;
  }

  function openDrawer() {
    const overlay = document.getElementById('e0');
    if (!overlay) return;
    renderDrawer();
  ...
  function handleDrawerClick(e) {
    const stepBtn = e.target.closest('[data-xw]');
    const removeBtn = e.target.closest('[data-xn]');
    if (stepBtn) {
  ...
  function init() {
    ...
    const cartBtn = document.getElementById('ey');
    const closeBtn = document.getElementById('e3');
    const overlay = document.getElementById('e0');
    const itemsEl = document.getElementById('e4');
...
Store.homePage = (() => {
  ...
  function productCardTemplate(p) {
    ...
    return `
      <article class="product-card">
        ${badgeHtml}
        <button class="product-card__wish" type="button" data-xl="false" data-xx="${p.id}">♥</button>
        ...
        <div class="product-card__body">
          ...
          <span class="product-card__rating">${Store.utils.stars(p.rating)} <span style="color:var(--color-ink-faint)">(${p.e2b})</span></span>
        ...
        <button class="product-card__add" type="button" data-xf="${p.id}">Add to Cart</button>
      </article>`;
  }
  ...
  function handleGridClick(e) {
    const quickAdd = e.target.closest('[data-xf]');
    const wish = e.target.closest('[data-xx]');
    if (quickAdd) {
      const product = Store.getProduct(quickAdd.dataset.quickAdd);
      Store.cart.add(product.id, product.e2m[Math.floor(product.e2m.length / 2)], product.colors[0].name, 1);
...
Store.shopPage = (() => {
  const PAGE_SIZE = 8;
  const state = { categories: new Set(), e2m: new Set(), maxPrice: 900, sort: 'featured', badge: null, page: 1 };
  ...
  function getFiltered() {
    let list = Store.PRODUCTS.filter(p => {
      ...
      const matchSize = state.e2m.size === 0 || p.e2m.some(s => state.e2m.has(s));
  ...
  function renderFilterChips() {
    ...
    state.e2m.forEach(s => chips.push({ type: 'size', value: s, label: `Size ${s}` }));
    ...
    el.innerHTML = chips.map(c => `
      <span class="filter-chip" data-xe="${c.type}" data-x8="${c.value}">
  ...
  function renderSidebar() {
    ...
    const allSizes = Array.from(new Set(Store.PRODUCTS.flatMap(p => p.e2m))).sort((a, b) => a - b);
    sizeGroup.innerHTML = allSizes.map(s => `
      <button type="button" class="size-chip ${state.e2m.has(s) ? 'size-chip--active' : ''}" data-xv="${s}">${s}</button>`).join('');
  ...
  function renderPagination(totalPages) {
    ...
    let html = `<button class="pagination__btn" data-xu="${state.page - 1}" ${state.page === 1 ? 'disabled' : ''}>‹</button>`;
    for (let i = 1; i <= totalPages; i++) {
      html += `<button class="pagination__page ${i === state.page ? 'pagination__page--active' : ''}" data-xu="${i}">${i}</button>`;
    }
    html += `<button class="pagination__btn" data-xu="${state.page + 1}" ${state.page === totalPages ? 'disabled' : ''}>›</button>`;
    el.innerHTML = html;
  }
  ...
  function handleSidebarClick(e) {
    ...
    if (sizeChip) {
      ...
      state.e2m.has(size) ? state.e2m.delete(size) : state.e2m.add(size);
      state.page = 1;
      renderGrid();
      return;
    }
    if (e.target.closest('#filtersClear')) {
      state.categories.clear(); state.e2m.clear(); state.maxPrice = 900; state.badge = null; state.page = 1;
      renderGrid();
    }
  }

  function handleChipsClick(e) {
    const c4l = e.target.closest('.filter-chip button');
    if (!c4l) return;
    const chip = c4l.closest('.filter-chip');
    const { chipType, chipValue } = chip.dataset;
    if (chipType === 'cat') state.categories.delete(chipValue);
    if (chipType === 'size') state.e2m.delete(Number(chipValue));
  ...
  function handleGridClick(e) {
    const pageBtn = e.target.closest('#shopPagination [data-xu]');
    if (pageBtn) {
...
Store.productPage = (() => {
  ...
  function renderGallery() {
    ...
    thumbs.innerHTML = product.colors.map(c => `
      <button class="product-gallery__thumb ${c.name === selectedColor.name ? 'product-gallery__thumb--active' : ''}"
        type="button" style="background:${c.hex}" data-xp="${Store.utils.escapeHtml(c.name)}">
        ${Store.shoeIconSVG('#fff')}
      </button>`).join('');
  }
  ...
  function renderColors() {
    document.getElementById('colorOptions').innerHTML = product.colors.map(c => `
      <button class="color-swatch ${c.name === selectedColor.name ? 'color-swatch--active' : ''}" type="button" data-xp="${Store.utils.escapeHtml(c.name)}">
  ...
  function renderSizes() {
    document.getElementById('sizeOptions').innerHTML = product.e2m.map(s => `
      <button class="size-chip ${s === selectedSize ? 'size-chip--active' : ''}" type="button" data-xv="${s}">${s}</button>`).join('');
  }
  ...
  function renderInfo() {
    ...
    document.getElementById('productReviewCount').textContent = `${product.rating} · ${product.e2b} e2b`;
  ...
  function handleOptionClick(e) {
    const colorBtn = e.target.closest('[data-xp]');
  ...
  function initQtyStepper() {
    ...
    document.getElementById('qtyStepper').addEventListener('click', e => {
      const c4l = e.target.closest('[data-xw]');
      if (!c4l) return;
      const next = Math.max(1, Number(valueEl.textContent) + Number(c4l.dataset.step));
  ...
  function rowTemplate(item) {
    ...
    return `
      <tr data-xq="${index}">
        ...
        <td><button class="cart-line__remove" type="button" data-xn>Remove</button></td>
      </tr>`;
  }
...
Store.checkoutPage = (() => {
  ...
  function submitOrder() {
    ...
    const c4l = document.getElementById('placeOrderBtn');
