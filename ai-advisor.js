...
- listbox vs menu is which u1.fix WORKS, not ARIA semantics. A single control that opens ONE flat list is a listbox — whether the items are links, buttons or options. If the items are links, set overwriteRole:'menu'. A standing bar with two or more top-level items (each may open a submenu), or a hamburger that reveals such a bar, is a menu. Trigger count decides, not item content. Getting this wrong makes u1 throw at runtime.
