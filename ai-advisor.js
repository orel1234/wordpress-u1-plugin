...
- tabs vs menu is decided the same way — by what pressing DOES, never by the word in the class name: tabs = a panel swapped in one fixed place; the content changes, the reader stays. menu = the items lead AWAY (navigate) or drop panels of links. role="tablist" or a tab-flavoured class over panels that navigate is still a MENU; sibling buttons swapping one region with no role at all are still TABS.
- listbox vs menu is decided by what the ITEMS DO, never by counting triggers: listbox = the items SELECT A VALUE and the trigger's text changes to the chosen item (a replaced <select> is the classic case). menu = the items NAVIGATE or run actions — including a single button that opens a dropdown of links ("My account" → Orders/Returns/Settings is a MENU even with one trigger). Trigger count does not decide. Getting this wrong makes u1 throw at runtime.
...
- "openedVia" tells you which kind of component you are looking at, and it is usually decisive:
  · a control with aria-haspopup, plus a list it controls → listbox if the items pick a value, menu if they navigate or act (real hrefs are the tell)
